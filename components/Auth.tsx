...
const Auth: React.FC<AuthProps> = ({ onAuthComplete }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isLogin) {
      ...
      const existing = storageService.getAllUsers().find(u => u.email === email);
      if (existing) {
      ...
      <div className="max-w-md w-full bg-[#0f172a] border border-slate-800 p-10 rounded-[32px] shadow-2xl relative overflow-hidden transition-all duration-500">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center mb-6">
             <Logo className="h-28" variant="full" />
          </div>
          <h2 className="text-xl font-black text-white tracking-widest uppercase mt-4">
            {isLogin ? 'Employee Portal' : 'Register Profile'}
          </h2>
          <p className="text-slate-600 text-[10px] font-bold uppercase tracking-[0.3em] mt-2 italic">Secured Intelligence Node</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {isLogin ? 'Establish Connection' : 'Register Corporate Identity'}
          </button>
        </form>
        ...
        <div className="mt-10 text-center">
          ...
          >
            {isLogin ? "Join the lab workforce" : "Return to security gate"}
          </button>
        </div>
      </div>
    </div>
